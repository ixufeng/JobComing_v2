...
function ShowMap(objname, options) {
	...
	cr.addCopyright({
		id: 1,
		content: "<a href='http://www.88148.com' style='font-size:12px;'>律法网</a>",
		bounds: bs
	});
	...
	if(this._ismove == "1") {
		...
		var label = new BMap.Label("拖拽到您的位置", {
	...
	if(this._ismove == "0") {
		...
		var opts = {
				width: 250,
				height: 110,
				title: "<font color=green size=3>" + this._title + "</font>"
			} //窗口标题
		var infotxt = "<table border='0'><tr><td valign='top'>"; //窗口内容
		if(this._pic != "") {
			infotxt += "<img src='" + this._pic + "' id='picid' style='float:left;margin-right:5px;padding-top:8px;' width=50>";
		}
		infotxt += "</td><td><p style='font-size:12px;line-height:16px;padding-top:8px;'>";
		if(this._lawfirm != "") {
			infotxt += "<b>律所：</b>" + this._lawfirm + "<br/>";
		};
		if(this._addr != "") {
			infotxt += "<b>地址：</b>" + this._addr + "<br/>";
		};
		if(this._tel != "") {
			infotxt += "<b>电话：</b>" + this._tel + "<br/>";
		};
		if(this._user != "") {
			infotxt += "<b>主任：</b>" + this._user + "<br/>";
		};
		infotxt += "</p></td></tr></table>";

		//显示文本标题
		var label2 = new BMap.Label(this._title, {
			offset: new BMap.Size(20, -15)
		});
		label2.setStyle({
			backgroundColor: "red",
		...
		marker.addEventListener("mouseover", function() {
			...
			document.getElementById('picid').onload = function() {
				infoWindow.redraw();
			}
		});
	}
